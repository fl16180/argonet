...
def beat_gft_array(table, metric, states):
    ...
    for st in states:
        ...
        for i, mod in enumerate(['ARGO','Net','ARGONet']):
            counter = 0
...
def main():
    ...
    table = table[table.Model.isin(['AR52','ARGO','Net','ARGONet'])]
    ...
    counts1 = metrics_pie(table, 'ARGO', 'AR52', states)
    ...
    argo1_seasons = get_all_seasons(table, 'ARGO', 'PEARSON').reshape(-1, 1)
    ...
    print 'ARGO:\t{0} ({1}-{2})'.format(np.percentile(argo2_seasons, 50),
    ...
    argo1_seasons = get_all_seasons(table, 'ARGO', 'RMSE').reshape(-1, 1)
    ...
    argo1_seasons = get_all_seasons(table, 'ARGO', 'MAPE').reshape(-1, 1)

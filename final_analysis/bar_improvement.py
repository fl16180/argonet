...
def main():
    ...
    rmse_diffs = [float(table_rmse[(table_rmse.Model == 'ARGO(gt,ath)') & (table_rmse.State == st)]['ARGONet Period'].values[0]) \
    ...
    # get list of differences between RMSEs
    rmse_diffs2 = [table2_rmse[(table2_rmse.Model == 'ARGO(gt,ath)') & (table2_rmse.State == st)]['ARGONet Period'].values[0] \
    ...
    for st in states_rmse_order2:
        tmp = table2[(table2.Metric == 'RMSE') & (table2.Model == 'ARGO(gt,ath)') & (table2.State == st)]['ARGONet Period'].values[0]

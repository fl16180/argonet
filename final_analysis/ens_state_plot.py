...
def main_plot(STATES, page, ids):
    ...
    for i, state in enumerate(STATES):
        ...
        models = pd.read_csv(config.STATES_DIR + '/{0}/top_ens_preds.csv'.format(state), parse_dates=[0])
    ...
    fig.savefig(config.HOME_DIR + '/final_analysis/ens_states_{0}.png'.format(page), format='png', dpi=300)
